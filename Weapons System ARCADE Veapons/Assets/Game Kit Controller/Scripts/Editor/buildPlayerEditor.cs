...
public class buildPlayerEditor : Editor
{
	...
	void showTemporalSettingsInfoListElement (SerializedProperty list)
	{
		...
		if (list.FindPropertyRelative ("useBoolState").boolValue) {
			EditorGUILayout.PropertyField (list.FindPropertyRelative ("boolState"), new GUIContent (""));
			//, new GUIContent (fieldName));
		}

		if (list.FindPropertyRelative ("useFloatValue").boolValue) {
			EditorGUILayout.PropertyField (list.FindPropertyRelative ("floatValue"), new GUIContent (""));
			//, new GUIContent (fieldName));
		}

		if (list.FindPropertyRelative ("useStringValue").boolValue) {
			EditorGUILayout.PropertyField (list.FindPropertyRelative ("stringValue"), new GUIContent (""));
			//, new GUIContent (fieldName));
		}

		if (list.FindPropertyRelative ("useRegularValue").boolValue) {
			EditorGUILayout.PropertyField (list.FindPropertyRelative ("regularValue"), new GUIContent (""));
			//, new GUIContent (fieldName));
		}
	}
